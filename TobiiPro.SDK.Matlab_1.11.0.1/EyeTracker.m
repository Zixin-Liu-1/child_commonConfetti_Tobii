...
%                tracker.stop_gaze_data();
        ...
        % Get Eye Openness Data for the Eye Tracker
        % Returns: array with instances of class <Gaze/EyeOpennessData.html EyeOpennessData>.
        %
        % <include>SampleCode/GetEyeOpennessData.m</include>
        %
        function output = get_eye_openness_data(tracker, mode)
            if nargin == 1
               mode = 'class';
            end

            eye_openness_data = tracker.APIcall('GetEyeOpennessData', tracker.CoreEyeTracker);

            if isfield(eye_openness_data, 'error')
               output = StreamError(eye_openness_data);
               tracker.stop_eye_openness_data();
               return
            end

            if strcmp(mode,'flat')
                output = eye_openness_data;
                return
            end

            data_size = size(eye_openness_data.device_time_stamp, 1);

            if data_size > 0
                output(data_size, 1) = EyeOpennessData;
                for i=1:data_size
                    output(i) = EyeOpennessData(eye_openness_data.device_time_stamp(i),...
                        eye_openness_data.system_time_stamp(i),...
                        eye_openness_data.left_eye_validity(i),...
                        eye_openness_data.left_eye_openness_value(i),...
                        eye_openness_data.right_eye_validity(i),...
                        eye_openness_data.right_eye_openness_value(i));
                end
            else
                output = [];
            end
        ...
        %
        %
        function stop_eye_openness_data(tracker)
            tracker.APIcall('StopEyeOpennessData', tracker.CoreEyeTracker);
        end
