...
% Uninstruct mode detect
% -----------------------
isUninstruct = isequal(taskParam.gParam.taskType, 'HamburgUninstruct');

% In the uninstruct condition, catch trials (generated by al_cannonData)
% are reinterpreted: instead of showing the cannon aim during prediction,
% catch trials show the outcome before the prediction is made.
...
for i = 1:trial
    ...
    % Uninstruct mode: on catch trials, show outcome BEFORE prediction as a
    % substitute for showing cannon
    % -------------------------------------------------------------------
    % In the uninstruct condition, catch trials (from al_cannonData) are
    % reinterpreted: instead of showing the cannon, the outcome is revealed
    % before the participant places their prediction.
    if isUninstruct && taskData.catchTrial(i) == 1

        if isequal(taskParam.trialflow.shot, 'static')

            % Draw circle, confetti cloud, fixation, and outcome
        ...
        elseif isequal(taskParam.trialflow.shot, 'animate cannonball')
            ...
            background = false;
            taskData = al_confetti(taskParam, taskData, i, background, GetSecs());

        end

        % Brief fixation cross between catch trial outcome and prediction
        al_fixationPhase(taskParam)
        Screen('DrawingFinished', taskParam.display.window.onScreen);
        Screen('Flip', taskParam.display.window.onScreen);
        WaitSecs(0.5);

        % Reset RT timestamp after catch trial display
        initRT_Timestamp = GetSecs();
